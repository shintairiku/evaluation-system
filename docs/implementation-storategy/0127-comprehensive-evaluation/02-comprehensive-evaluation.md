...
| 昇格フラグ | `evaluation_score_mapping`に基づく（例: 昇格/降格/対象外） |
...
## 6.5 複合条件ルール（AND/OR）の管理（必須・将来拡張前提）

昇格/降格候補の判定や、特定条件での上書き（例: 面談フラグ、雇用形態、ステージ等）は、今後 **`AND`条件だけでなく`OR`条件** で設定したいケースが発生する。

そのため、判定ルールは「単一の固定フォーム」ではなく、以下を満たす形で設計する。

- **条件式が`AND`/`OR`で表現できる**（例: `A以上 OR (B以上 AND 面談クリア)`）
- ルールが複数ある場合の **優先順位（`priority`）** を持てる
- ルール定義は`eval_admin`のみ編集可能（`admin`は閲覧のみ）
- Backendで一貫して評価し、フロント側に判定ロジックを分散させない

実装案:
- `evaluation_score_mapping`は「点数→ランク/デフォルト増減」に集中
- 昇格/降格/上書きなどの **複合条件** は別テーブル（例: `comprehensive_evaluation_rules`）にJSON（JSONB）で保持
  - UIは「ルールビルダー」で編集し、Backendはバリデーションと評価（ルールエンジン）を担当
...
- [ ] 昇格/降格などの判定条件を、将来要件を見越して`AND`/`OR`で表現・保存できる設計になっている（ルールビルダー対応）
