...
|---|---|---|---|---|
| v1.6 | 2026-02-26 | AI（Codex） | 評価期間終了フローを追加。`eval_admin`のみ期間終了、終了後スコア編集ロック。 | 🔄 レビュー中 |
...
- [x] `eval_admin`のみ評価期間を終了でき、終了時は期間ステータスのみ`completed`になる
- [x] 評価期間終了後、スコア変更系操作（自己評価/上司評価）は拒否される
