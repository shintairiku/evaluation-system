...
|---|---|---|---|---|
| v1.4 | 2026-02-18 | AI（Codex） | `self_assessments` / `supervisor_feedback` の実装状況を反映。モック列のDB可否を確定。総合評価データ取得を「単一集計API + 単一集計クエリ」に確定。 | 🔄 レビュー中 |

---

## TL;DR（結論）

- `self_assessments` / `supervisor_feedback` のDBスキーマとAPIはすでに利用可能
- 総合評価モックの主要列はDBから取得可能だが、以下は現行スキーマだけでは不足
  - `現在レベル`（`users.level` が未実装）
  - `コアバリュー最終評価`（360評価系の最終結果テーブル未接続）
  - 面談系フラグ（`leaderInterviewCleared` など、DB項目なし）
...
## 3. 実装済み前提（2026-02-18時点）

### 3.1 DB / API 反映済み

| 領域 | 状態 | 要点 |
|---|---|---|
| `self_assessments` | ✅ 実装済み | `self_rating_code`, `self_rating(0-100)`, `rating_data`, `status(draft/submitted/approved)` |
| `supervisor_feedback` | ✅ 実装済み | `supervisor_rating_code`, `supervisor_rating(0-100)`, `rating_data`, `action(PENDING/APPROVED)`, `status` |
| `evaluation_score_mapping` | ✅ 実装済み | 組織別 `rating_code -> score_value`（`SS/S/A/B/C/D`） |
| 総合評価専用API | ⏳ 未実装 | `/evaluation/goal-list-page` は存在するが総合評価行DTOは未提供 |
...
- `業績加重スコア = Σ(業績目標の supervisor_rating × goal.weight) / 100`
- `コンピテンシー加重スコア = Σ(コンピテンシー目標の supervisor_rating × goal.weight) / 100`
- `業績点 = 業績加重スコア / 10`
- `コンピテンシー点 = コンピテンシー加重スコア / 10`
- `合計点 = 業績点 + コンピテンシー点`
...
- [ ] `/admin-eval-list` がモックではなく総合評価APIから表示される
- [ ] 合計点/総合評価がDBデータで算出される
- [ ] モック列の不足項目は仕様通りに `-` / `未確定` 表示される
- [ ] `admin` は閲覧のみ、`eval_admin` は判定基準編集可能
- [ ] 一覧取得が単一集計クエリで動作し、N+1が発生しない
- [ ] 特例反映がAPI経由で永続化され、監査ログを追跡できる

---

## 11. 未決定事項 / 要確認

- `users.level` を追加するか、別テーブルで社員レベルを管理するか
