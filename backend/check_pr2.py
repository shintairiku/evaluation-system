...
async def check():
    async with AsyncSessionLocal() as session:
        result = await session.execute(text("SELECT u.id, u.name FROM users u WHERE u.email = 'shiruba.global@gmail.com'"))
        ...
        result = await session.execute(text("""
            SELECT pra.id, u_reviewee.name as reviewee_name, 
